...
import {models,options,fields} from "@/pages/Media/form_fields";
import {getMediaData} from "@/API/MediaAPIs";
...
export default {
  ...
  watch: {
    async id(val) {
      if (this.id)
        this.data = await getMediaData(val);
    },
    model:{
      deep:true,
      handler(value){
  ...
  methods: {
    ...
    async getData(){
      ...
      if (this.id)
        return this.data = await getMediaData(this.id);
      if (this.model)
