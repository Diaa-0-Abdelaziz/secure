<template>
  <div class="row cars-list">
    <div class="col-md-12">
      <h4 class="title">Business List <small class="text-muted ml-2">Browse all the system Business.</small></h4>
    ...
    <div class="col-4 px-3 " v-if="showAdd || editModel">
      <div class="card">
        <div class="card-body">
          <CreateBusiness v-if="showAdd" :in-list="true"
                            @onSubmit="$refs.table.getTableData()"
                            @updated="$refs.table.getTableData()" />
          <EditBusiness v-if="editModel" :in-list="true"
                          @onSubmit="$refs.table.getTableData()"
                          @updated="$refs.table.getTableData()" :model="editModel"/>
        </div>
      </div>
    </div>
  </div>
...
import {deleteBusiness, getBusinessList} from "@/API/BusinessAPIs";
...
export default {
  ...
  methods: {
    ...
    getData({page}) {
      return getBusinessList({'per-page': this.pagination.perPage, page})
        .then(response => {
