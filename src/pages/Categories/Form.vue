...
import {models,options,fields} from "@/pages/Categories/form_fields";
import {getCategoryData, getCategoryOptionsList} from "@/API/CategoriesAPIs";
...
export default {
  ...
  async mounted() {
    ...
    await this.getData();
    this.options.parent_idOptions = await getCategoryOptionsList();
    this.data.business_id = this.businessId;
    this.data.menu_id = this.menuId;
  },
  watch: {
    async id(val) {
      if (this.id)
        this.data = await getCategoryData(val);
    },
    model:{
      deep:true,
      handler(value){
  ...
  methods: {
    ...
    async getData(){
      ...
      if (this.id)
        return this.data = await getCategoryData(this.id);
      if (this.model)
