...
import {models,options,fields} from "@/pages/Hotels/form_fields";
import {getHotelData} from "@/API/HotelsAPIs";
...
export default {
  ...
  watch: {
    async id(val) {
      if (this.id)
        this.data = await getHotelData(val);
    },
    model:{
      deep:true,
      handler(value){
  ...
  methods: {
    ...
    async getData(){
      ...
      if (this.id)
        return this.data = await getHotelData(this.id);
      if (this.model)
