...
export default {
  ...
  methods: {
    ...
    async getData() {
      this.data = {...JSON.parse(this.emptyModel)};
      var settingData = await listBranchSetting({
        businessId: this.selectedBusinessId,
        branchId: this.selectedBranchId
      });
      let tabletSettings = _.filter(settingData, {key: AppSettings.TabletAppSettings});
      if (tabletSettings && tabletSettings.length)
        tabletSettings[0]?.data?.forEach(set => {
    ...
    modelToSubmitDataConverter() {
      ...
      return [{"key": AppSettings.TabletAppSettings, data}];
    },

    async setBranchId() {
      let loader = this.$loading.show();
      this.selectedBranchId = this.$store.state.user.business.filter(r => r.id === this.selectedBusinessId)[0].branches[0].id
      console.log("selectedBranchId", this.selectedBusinessId, this.selectedBranchId)
      await this.getData();
      loader.hide();
