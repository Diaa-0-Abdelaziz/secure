<template>
  <div class="row cars-list">
    <div class="col-md-12">
      <h4 class="title">Services List <small class="text-muted ml-2">Browse all the system Services.</small></h4>
        ...
        <div class="col-12">
          <data-table :get-data="getData"
            ...
            <div slot="media" slot-scope="{row}">
              <img v-if="row.media[0]" :src="row.media[0]['src']" class="img-thumbnail w-50"  />
            </div>
    ...
    <div class="col-4 px-3 " v-if="showAdd || editModel">
      <div class="card">
        <div class="card-body">
          <CreateService v-if="showAdd" :in-list="true" @onSubmit="$refs.table.getTableData()"/>
          <EditService v-if="editModel" :in-list="true" @onSubmit="$refs.table.getTableData()" :model="editModel"/>
        </div>
      </div>
    </div>
  </div>
...
import {deleteService, getServicesList} from "@/API/ServicesAPIs";
...
export default {
  ...
  methods: {
    ...
    getData({page}) {
      return getServicesList({'per-page': this.pagination.perPage, page})
        .then(response => {
