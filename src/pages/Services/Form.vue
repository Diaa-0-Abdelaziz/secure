...
import {models,options,fields} from "@/pages/Services/form_fields";
import {getServiceData} from "@/API/ServicesAPIs";
...
export default {
  ...
  watch: {
    async id(val) {
      if (this.id)
        this.data = await getServiceData(val);
    },
    model:{
      deep:true,
      handler(value){
  ...
  methods: {
    ...
    async getData(){
      ...
      if (this.id)
        return this.data = await getServiceData(this.id);
      if (this.model)
